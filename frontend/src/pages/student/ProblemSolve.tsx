...
export default function ProblemSolve() {
  ...
  const problem = problems.find((p) => p.id === id) ?? problems[2];
  const detail: ProblemDetail = useMemo(
    () => getDetail(problem.id, problem.title, problem.points),
    [problem.id, problem.title, problem.points]
  );
  ...
  return (
    <div className="flex-1 flex flex-col min-h-0 h-full text-[var(--ws-text)]">
      ...
      {/* ============ SOLUTION MODAL ============ */}
      {showSolution && (
        <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4" onClick={() => setShowSolution(false)}>
