        ...
        <header className="sticky top-0 z-50 h-16 flex-shrink-0 bg-[#f7f4eb]/80 backdrop-blur-xl border-b border-[#e5dac9] flex items-center px-4 lg:px-6 gap-3">
          ...
          <div className="relative pl-3 border-l border-[#e5dac9]">
            <button
              ...
              className={`flex items-center gap-2.5 rounded-xl px-1.5 py-1 transition-colors ${
            ...
            >
              ...
              <div className="hidden lg:block leading-tight text-left">
                <p className="text-[12.5px] font-semibold text-[#191919]">{user?.fullName}</p>
                <p className="text-[11px] text-[#8a8073]">{user?.role === 'instructor' ? 'Giảng viên' : 'Sinh viên'}</p>
              </div>
              <ChevronDown size={15} className={`hidden lg:block text-[#8a8073] transition-transform ${avatarOpen ? 'rotate-180' : ''}`} />
            </button>
